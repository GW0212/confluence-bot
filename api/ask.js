      ...
      for (const chunk of context.split(/(?=\n?=== \[)/)) {
        try {
          ...
          body = body
            ...
            .replace(/&lt;|&gt;/g, '')
            // 변경이력 문장 패턴 제거: "~에 의해 변경되었습니다", 파일명, 날짜+이름 조합 등
...
[절대 규칙 - 반드시 준수]
1. [기획서 원문]에 있는 내용만 사실로 인정하고, 답변은 원문에 적힌 사실에 기반해야 해.
...
6. 매우 중요 - 여러 페이지 종합: 질문이 특정 패치/버전/주제를 가리키면(예: "M58", "전투 시스템"), [사용 가능한 페이지 목록]에서 그 패치/주제와 관련된 페이지가 여러 개 있을 수 있다. 한두 페이지만 보고 끝내지 말고, 목록에 있는 관련 페이지를 전부 [기획서 원문]에서 찾아 누락 없이 종합해서 답하되, 각 페이지 내용은 핵심만 간결하게 요약해. 페이지가 다르면 섹션을 나눠서(## 또는 ### 사용) 각각 표시해.
...
10. 반드시 한국어로 답하고, 구조는 ## 대제목 [[출처]] / ### 소제목 / - 항목 / **핵심 수치**로 정리해. 절대 규칙: 항목(-) 사이, 소제목(###) 다음, 대제목(##) 다음에 빈 줄(\n\n)을 절대 넣지 마. 모든 항목은 빈 줄 없이 바로 이어서 써. 예시: "## 제목\n- 항목1\n- 항목2\n### 소제목\n- 항목3"
...
16. 매우 중요 - 페이지 내용 요약 시: "OO 페이지 내용을 정리해줘" 같은 질문에 "이 페이지는 ~와 관련된 내용을 담고 있습니다" 같은 추상적인 설명은 절대 하지 마. 반드시 [기획서 원문]에 있는 실제 수치·규칙·시스템 내용을 ## 섹션 / - 항목 구조로 구체적으로 정리해서 답해.
