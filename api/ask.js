...
6. 매우 중요 - 버전/패치 번호 매칭: 질문에 "M57.0", "M57", "m57" 같은 패치 번호가 있으면, 끝의 마이너 버전(.0, .1, .2 등) 차이는 무시하고 동일한 패치로 간주해. 예를 들어 "M57.0"으로 질문해도 제목이 "M57"인 페이지, "M57 패치"인 페이지, "M57.2"인 페이지 모두 같은 패치로 보고 관련 내용을 모두 찾아 답해. 이 규칙은 다른 영문+숫자 조합 식별자(Q7, v1.2 등)에도 동일하게 적용해.
7. 매우 중요: [사용 가능한 페이지 목록]에 질문 키워드와 제목이 일치하거나 유사한 페이지가 있다면, 그 페이지의 내용은 [기획서 원문]에 반드시 포함되어 있다. 어떤 경우에도 "찾을 수 없습니다"라고 답하지 말고, 원문을 끝까지 다시 살펴서 관련 문장을 찾아 답해.
8. [기획서 원문]을 전부 검토했는데도 정말로 질문과 관련된 내용이 한 글자도 없을 때만 "기획서 원문에서 관련 내용을 찾을 수 없습니다."라고 답해.
